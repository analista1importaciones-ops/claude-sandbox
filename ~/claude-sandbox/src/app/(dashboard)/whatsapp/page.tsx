      ...
      <div className="w-72 border-r border-gray-100 flex flex-col bg-white">
        <div className="px-4 py-4 border-b border-gray-100">
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            <span className="text-xs text-gray-500 capitalize">{status === 'connected' ? 'Conectado' : status === 'connecting' ? 'Conectando...' : 'Desconectado'}</span>
          </div>
          {status === 'disconnected' && (
        ...
        <div className="flex-1 overflow-y-auto">
          {conversations.length === 0 && status === 'connected' && (
            <p className="text-xs text-gray-400 text-center mt-8">Los chats aparecerán aquí</p>
          )}
          {conversations.map((c) => (
            <button
              key={c.jid}
              onClick={() => setSelectedJid(c.jid)}
              className={`w-full text-left px-4 py-3 border-b border-gray-50 hover:bg-gray-50 ${selectedJid === c.jid ? 'bg-orange-50' : ''}`}
            >
              <p className="text-sm font-medium text-gray-800">{c.contactName || c.jid.replace('@s.whatsapp.net', '')}</p>
